...
def simple_collate_fn(features: List[Dict[str, Any]], pad_token_id: int) -> Dict[str, torch.Tensor]:
    """简单的 collate 函数，用于手动创建 DataLoader"""
    ...
    for f in features:
        ...
        batch["input_ids"].append(input_ids + [pad_token_id] * pad_len)
        batch["attention_mask"]. append([1] * len(input_ids) + [0] * pad_len)
...
def evaluate_token_accuracy(
    ...
    collate_fn = partial(simple_collate_fn, pad_token_id=tokenizer.pad_token_id)
