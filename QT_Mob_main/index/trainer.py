...
class Trainer(object):
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        for batch_idx, (data,neighbor) in enumerate(iter_data):
    ...
    def fit(self, data):
        ...
        for epoch_idx in range(self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if train_loss < self.best_loss:# 保存最好的loss
                    self.best_loss = train_loss
                    self._save_checkpoint(epoch=epoch_idx, ckpt_file=self.best_loss_ckpt)

                if collision_rate < self.best_collision_rate: # 保存最好的collision_rate
